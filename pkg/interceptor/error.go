...
func ErrorCodesInterceptor(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (res interface{}, err error) {
	...
	switch {
	case commonErrors.IsCommonError(err):
		commEr := commonErrors.GetCommonError(err)
	...
	default:
		// если ошибка grpc (к примеру от другого сервиса), то возвращаем ее как она есть
		var defErr GRPCStatusInterface
		if errors.As(err, &defErr) {
			return nil, defErr.GRPCStatus().Err()
		} else {
